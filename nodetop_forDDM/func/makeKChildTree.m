...
function tree = makeKChildTree(numLevels,k)

% this function makes an undirected tree graph, where each parent has k children

% input: numLevels, the number of "levels" in a tree constructed from an initial root node (including the root node level)
% input: k, the number of children to be generated at each level
% output: tree, adjacency matrix of constructed k-child tree graph



% print status
fprintf('Constructing k-child tree...');
